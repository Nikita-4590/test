...
function confirmChangeRenkeiDate(requestId) {
	if (isUnset(me.dapps.global['request.change_renkei_confirm_box'])) {
		
		me.dapps.global['request.change_renkei_confirm_box'] = new me.dapps.box({
			auto_hide : false,
			title : '変更しますか？',
			close_button : false,
			loading_text : '読み込み中。。。',
			button : {
				...
				list : [ {
					...
					action : function(targetBox) {
						...
						targetBox.main.find('#change-renkei-form').submit();
					}
				} ]
			}
		});
		// fix bug #2879
		me.dapps.global['request.change_renkei_confirm_box'].submitAgent = {
				isSubmitting : false,
				theForm : null,// form
				theMask : null,// mask 
	...
	me.dapps.global['request.change_renkei_confirm_box'].showFromUrl({
		url : me.dapps.global['url.confirm_change_renkei_date'],
		method : 'post',
		data : {
		...
		callback : function(targetBox) {
			targetBox.main.find('#change-renkei-form').validator();
			
			var theAgent = me.dapps.global['request.change_renkei_confirm_box'].submitAgent;
			theAgent.setSubmitForm(targetBox.main.find('#change-renkei-form'));
			theAgent.injectSubmit();
			
			targetBox.main.find('#change-renkei-form').ajaxForm({
				dataType : 'json',
				...
				error : function(e, err) {
					...
					me.dapps.global['request.change_renkei_confirm_box'].submitAgent.endSubmit();// Alert to submitAgent that Submit have finished
			    }
			});

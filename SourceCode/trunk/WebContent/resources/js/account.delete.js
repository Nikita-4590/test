...
function confirmDelete(accountType, accountId) {
	if (isUnset(me.dapps.global['account.delete_confirm_box'])) {
		
		me.dapps.global['account.delete_confirm_box'] = new me.dapps.box({
			auto_hide : false,
			title : '削除しますか？',
			close_button : false,
			loading_text : '読み込み中。。。',
			button : {
				...
				list : [ {
					...
					action : function(targetBox) {
						...
						targetBox.main.find('#delete-account-form').submit();
					}
				} ]
			}
		});
		// fix bug #2879
		me.dapps.global['account.delete_confirm_box'].submitAgent = {
				isSubmitting : false,
				theForm : null,// form
				theMask : null,// mask 
	...
	me.dapps.global['account.delete_confirm_box'].showFromUrl({
		url : me.dapps.global['url.confirm_delete_account'],
		method : 'post',
		data : {
		...
		callback : function(targetBox) {
			...
			var theAgent = me.dapps.global['account.delete_confirm_box'].submitAgent;
			theAgent.setSubmitForm(targetBox.main.find('#delete-account-form'));
			theAgent.injectSubmit();
			
			targetBox.main.find('#delete-account-form').ajaxForm({
				dataType : 'json',
				...
				error : function(e, err) {
					...
					var messageId = me.dapps.global['message.delete.general'];

					if (e.status == 403) {
						me.dapps.global['message.delete.forbidden'];
					}
					...
					me.dapps.global['account.delete_confirm_box'].submitAgent.endSubmit();// Alert to submitAgent that Submit have finished
			    }
			});
		...
		error : function(box, e) {
			...
			if (e.status == 403) {
				...
				me.dapps.global['message.delete.not_found'];
			}
